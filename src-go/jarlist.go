...
// 自己创建jvm，传递jar包列表，使用 -Djava.class.path=path1:path2

func packjvmargs() []string {
	var args = []string{} // "-verbose", "-Xms16m", "-Xmx228m"
	// args = append(args, "-Djava.class.path=/Users/gzleo/aprog/kmpchat/desktopApp/build/libs/desktopApp-jvm.jar")
	if true {
		// return args
	}
	...
	var gradle_cache = os.Getenv("GRADLE_CACHE")
	if gradle_cache == "" {
		// try default
		gradle_cache = home + "/.gradle/cache/modules-2/files-2.1"
	}

	var jarlst = jarlst_kt2_kmp010610
	jarlst = strings.ReplaceAll(jarlst, "$(HOME)", home)
	jarlst = strings.ReplaceAll(jarlst, "$(GRADLE_CACHE)", gradle_cache)

	var lst = strings.Split(jarlst, " ")
	var lastarg = ""
	var mainjar = ""
	for _, arg := range lst {
		if arg == "" {
		...
		if lastarg == "-cp" {
			var l = strings.Split(arg, ":")
			for _, x := range l {
				if false {
					args = append(args, "-Djava.class.path="+x)
				}
			}
			mainjar = "-Djava.class.path=" + l[0]
